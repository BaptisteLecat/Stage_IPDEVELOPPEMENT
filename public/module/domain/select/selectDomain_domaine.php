...
/**
 *  File all on AJAX to load the DNSRecord link to a Domain.
 */
    ...
    foreach ($list_client as $client) {
        foreach ($client->getList_Domains() as $domain) {
            if ($domain->getId() == $idDomain) {
                $clientId = $domain->getClientObject()->getId();
...
$response = ["success" => $success, "list_DNSRecord" => $list_DNSRecord, "clientId" => $clientId];
