function DNS_modal() {
    if (selectedDomain_index != null) { //Si un domaine est sélectionné.
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                document.getElementsByName("type-selector")[0].addEventListener("change", function (event) {
                    ...
                    if (event.target.selectedOptions[0].innerText == "MX") {
                        displayMXPriorityInput(); //Affichage de l'input number priority
                        type_isMX = true;
                    } else {
        ...
        xhr.send("view=form&form=insertDNS&idDomain=" + encodeURI(idDomain) + "&controller=" + encodeURI(controller));
    }
}
